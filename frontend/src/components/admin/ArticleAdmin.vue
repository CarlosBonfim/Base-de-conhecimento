<template>
  <div class="article-admin">
    <b-form>
        <input id="article_id" type="hidden" v-model="article.id" />
            <b-form-group label="Nome" label-for="article-name">
                <b-form-input id="article.name" type="text" v-model="article.name" required placeholder="Insira o nome do Artigo" :readonly="mode === 'remove'"/>
            </b-form-group>
            ...
            <b-form-group v-if="mode === 'save'" label="Categoria:" label-for="article-categoryId">
                <b-form-select  id="article-categoryId" :options="categories" v-model="article.categoryId" class="mt-2" />
            </b-form-group>

            <b-form-group v-if="mode === 'save'" label="Autor:" label-for="article-userId">
                <b-form-select  id="article-userId" :options="users" v-model="article.userId" class="mt-2" />
            </b-form-group>
    ...
    <b-table hover striped :items="this.articles" :fields="fields">
      <template slot="actions" slot-scope="data">
        <b-button variant="warning" @click="loadArticle(data.item)" class="mr-1 mt-1">
          <i class="fa fa-pencil"></i>
        </b-button>
        <b-button  variant="danger"  @click="loadArticle(data.item, 'remove')"  class="mr-1 mt-1">
...
export default {
  ...
  methods: {
    ...
    remove() {
      ...
      axios
        .delete(`${baseApiUrl}/articles/${id}`, this.article)
        .then(() => {
          this.$toasted.global.defaultSuccess();
          this.reset();
        })
